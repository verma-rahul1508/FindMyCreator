...
const externalProfileUrl = (value: string | null) => {
...
export default function AdminCreatorDetailPage() {
  ...
  const [sendingAccountSetup, setSendingAccountSetup] = useState(false);
  const [accountSetupNotice, setAccountSetupNotice] = useState('');
  ...
  const sendAccountSetup = async () => {
    if (sendingAccountSetup) return;
    const supabase = getSupabaseClient();
    const { data: sessionData } = supabase ? await supabase.auth.getSession() : { data: { session: null } };
    if (!sessionData.session?.access_token) { setError('Your administrator session has expired.'); return; }
    setSendingAccountSetup(true); setError(''); setAccountSetupNotice('');
    const response = await fetch(`/api/admin/creators/${creatorId}/account-setup`, { method: 'POST', headers: { Authorization: `Bearer ${sessionData.session.access_token}` }, cache: 'no-store' });
    const result = await response.json().catch(() => ({})) as { error?: string };
    setSendingAccountSetup(false);
    if (!response.ok) { setError(result.error || 'We could not send the password setup link.'); return; }
    setAccountSetupNotice('Password setup link sent. The creator can set a password and sign in.');
  };
  ...
  return <section className="mx-auto w-full max-w-6xl px-5 py-9 sm:px-8 sm:py-12 lg:px-10"><Link href="/admin/creators" className="text-sm font-semibold text-[#6330dc]">← Back to Creators</Link>
    ...
    {accountSetupNotice && <p role="status" className="mt-4 rounded-xl bg-[#f3fbf6] px-4 py-3 text-sm text-[#26754b]">{accountSetupNotice}</p>}
    ...
    <Section title="Social Platforms"><div className="space-y-4">{detail.platforms.length ? detail.platforms.map((platform) => { const profileUrl = externalProfileUrl(platform.profileUrl); return <div key={platform.id} className="rounded-xl border border-[#ece9f1] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-semibold capitalize">{platform.platform} {platform.isPrimary && <span className="ml-2 rounded-full bg-[#f0eaff] px-2 py-1 text-xs text-[#6330dc]">Primary</span>}</p><p className="mt-1 break-all text-sm text-[#626a7a]">{profileUrl ? <a href={profileUrl} target="_blank" rel="noreferrer" className="font-medium text-[#6330dc] underline underline-offset-2">{platform.profileUrl}</a> : platform.profileUrl || 'No profile URL'}{platform.username ? ` · @${platform.username}` : ''}</p></div><p className="text-sm font-semibold">{formatNumber(platform.audienceCount)} followers</p></div><AnalyticsBlock title="Instagram Analytics" values={platform.instagramAnalytics}/><AnalyticsBlock title="Facebook Analytics" values={platform.facebookAnalytics}/><AnalyticsBlock title="YouTube Analytics" values={platform.youtubeAnalytics}/></div>; }) : <p className="text-sm text-[#697080]">No connected platforms.</p>}</div></Section>
