...
export default function AdminCreatorOnboardingPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (saveError || !data) throw new Error(saveError?.message || 'We could not save this creator.');
      ...
      else { setDraftId(data.draftId); router.replace(`/admin/creators/new?draft=${data.draftId}`); setMessage(data.creatorId ? 'Saved. This creator is now available for review.' : 'Draft saved. Complete Basic Information to create the creator record.'); }
  ...
  return <section className="mx-auto w-full max-w-6xl px-5 py-9 sm:px-8 sm:py-12 lg:px-10"><Link href="/admin/creators" className="text-sm font-semibold text-[#6330dc]">← Creator Management</Link><div className="mt-5 flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-[#7C3AED]">Manual onboarding</p><h1 className="mt-2 text-4xl font-semibold tracking-[-0.06em]">{requestedCreator ? 'Edit creator' : 'Add a creator'}</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-[#626a7a]">{requestedCreator ? 'Update the creator details below and save your changes.' : 'Capture creator details from your call. Save at any point; incomplete sections remain in this private admin draft.'}</p></div>{creatorId && <Link href={`/admin/creators/${creatorId}`} className="rounded-lg border border-[#dedbe5] px-4 py-2.5 text-sm font-semibold">Open Review Profile</Link>}</div>
    <div className="mt-8 space-y-6">
      <section className="rounded-2xl border border-[#e8e7eb] bg-white p-5 sm:p-7"><h2 className="text-xl font-semibold">Basic Information</h2><p className="mt-1 text-sm text-[#697080]">{requestedCreator ? 'Update the saved contact and personal information.' : 'These fields are required before the creator record can be created. No invite or signup link is sent.'}</p><div className="mt-5 grid gap-4 sm:grid-cols-2">{([['Full name','fullName','text'],['Email','email','email'],['Phone number','phoneNumber','tel'],['Current city','currentCity','text'],['Date of birth','dateOfBirth','date'],['Gender','gender','select']] as const).map(([label,key,type]) => <Field key={key} label={label}>{type === 'select' ? <select value={payload.basic.gender} onChange={(event) => update('basic',{ gender:event.target.value })} className={inputClass}><option value="">Select gender</option>{['Male','Female','Non-binary','Prefer not to say'].map((option) => <option key={option}>{option}</option>)}</select> : <input type={type} value={payload.basic[key]} onChange={(event) => update('basic',{ [key]:event.target.value })} className={inputClass}/>}</Field>)}</div><Field label="Notes (optional)"><textarea value={payload.basic.more} onChange={(event) => update('basic',{ more:event.target.value })} rows={3} className={`${inputClass} resize-none`} /></Field></section>
      <section className="rounded-2xl border border-[#e8e7eb] bg-white p-5 sm:p-7"><h2 className="text-xl font-semibold">Creator Identity</h2><div className="mt-5 grid gap-4 sm:grid-cols-2"><Field label="Display name"><input value={payload.identity.displayName} onChange={(event) => update('identity',{ displayName:event.target.value })} className={inputClass}/></Field><Field label="Creator type"><select value={payload.identity.creatorType} onChange={(event) => update('identity',{ creatorType:event.target.value })} className={inputClass}><option value="">Select creator type</option><option value="content_creator">Content Creator</option><option value="influencer">Influencer</option><option value="ugc_creator">UGC Creator</option><option value="digital_creator">Digital Creator</option><option value="other">Other</option></select></Field>{payload.identity.creatorType === 'other' && <Field label="Creator type"><input value={payload.identity.creatorTypeOther} onChange={(event) => update('identity',{ creatorTypeOther:event.target.value })} className={inputClass}/></Field>}</div><Field label="Bio"><textarea value={payload.identity.bio} onChange={(event) => update('identity',{ bio:event.target.value })} rows={3} className={`${inputClass} resize-none`} /></Field><p className="mt-5 text-sm font-semibold">Languages</p><div className="mt-3 flex flex-wrap gap-2">{languages.map((language) => <button key={language} type="button" onClick={() => update('identity',{ languages:toggle(payload.identity.languages,language) })} className={`rounded-full border px-3 py-1.5 text-sm ${payload.identity.languages.includes(language) ? 'border-[#6330dc] bg-[#f1ebff] text-[#6330dc]' : 'border-[#dedbe5]'}`}>{language}</button>)}</div></section>
