...
const normalizeOther = (values: string[] | undefined, other?: string) => (values || []).map((value) => value === 'Other' ? other?.trim() || '' : value).filter(Boolean);

function toSocialAccount(account: PublicSocialAccount, index: number): SocialAccount {
  const platform = account.platform.trim();
  const key = platform.toLowerCase();
  const insights = account.insights;
  const audience = {
    locations: {
      countries: (insights?.countries || []).map((item, itemIndex) => ({ id: `country-${index}-${itemIndex}`, name: item.name || '', percentage: asText(item.percentage) })),
      cities: (insights?.cities || []).map((item, itemIndex) => ({ id: `city-${index}-${itemIndex}`, name: item.name || '', percentage: asText(item.percentage) })),
    },
  };
...
export default function PublicCreatorProfilePage() {
  ...
  if (loadError || !profile) return <main className="grid min-h-screen place-items-center bg-[#fbfaff] px-6 text-center"><div className="max-w-md"><h1 className="text-2xl font-semibold tracking-[-0.05em] text-black">We couldn&apos;t load this creator profile</h1><p className="mt-3 text-sm leading-6 text-[#626a7a]">The link may be incorrect or the profile is no longer available.</p></div></main>;

  const displayName = profile.displayName?.trim() || 'Creator';
  const initials = displayName.split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]).join('').toUpperCase();
  const identity: Identity = { profile_photo_url: null, display_name: profile.displayName || null, username: profile.username || null, bio: profile.bio || null, languages: profile.languages || [], creator_type: profile.creatorType && creatorTypeLabels[profile.creatorType] ? profile.creatorType : profile.creatorType ? 'other' : null, creator_type_other: profile.creatorType && creatorTypeLabels[profile.creatorType] ? null : profile.creatorType || null };
  const content: ContentDraft = { primary: profile.content?.primaryNiche || '', otherNiches: normalizeOther(profile.content?.otherNiches, profile.content?.otherNichesOther), contentFormats: normalizeOther(profile.content?.contentFormats, profile.content?.contentFormatsOther), contentStyles: normalizeOther(profile.content?.contentStyles, profile.content?.contentStylesOther) };
  ...
  const creatorType = profile.creatorType ? creatorTypeLabels[profile.creatorType] || profile.creatorType : '';
