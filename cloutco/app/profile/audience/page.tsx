...
function ProgressCard() {
  const steps = [['Basic Information', 'Personal details', 'done'], ['Creator Identity', 'Your creator persona', 'done'], ['Content & Niche', 'What you create', 'done'], ['Social Platforms', 'Where you create', 'done'], ['Audience', 'Your audience insights', 'current'], ['Portfolio', 'Showcase your work', '']];
  return <aside className="order-2 rounded-2xl border border-[#e8e7eb] bg-white p-5 shadow-[0_7px_20px_rgba(50,40,80,0.03)] xl:order-1 xl:sticky xl:top-5"><div className="flex items-center justify-between"><h2 className="text-sm font-semibold">Complete your profile</h2><span className="text-xs text-[#596177]">5 of 6</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-[#ececf1]"><span className="block h-full w-[83%] rounded-full bg-[#7034e8]" /></div><ol className="mt-5 space-y-1.5">{steps.map(([title, subtitle, state], index) => <li key={title} className={`flex items-center gap-3 rounded-xl px-2 py-2 ${state === 'current' ? 'bg-[#f3efff]' : ''}`}><span className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-xs font-semibold ${state === 'done' ? 'bg-[#57b8ae] text-white' : state === 'current' ? 'bg-[#6731dc] text-white shadow-[0_5px_12px_rgba(103,49,220,0.22)]' : 'border border-[#ccd1dd] bg-white text-[#495162]'}`}>{state === 'done' ? '✓' : index + 1}</span><span><strong className="block text-xs font-medium">{title}</strong><small className="block text-[0.66rem] text-[#677082]">{subtitle}</small></span></li>)}</ol></aside>;
}
