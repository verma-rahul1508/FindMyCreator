...
type InstagramAudience = { followers?: string; followerGrowth?: string; women?: string; men?: string; ages?: Record<string, string>; topAgeRanges?: string[]; topCities?: string[]; locations?: { countries?: Array<{ id?: string; name?: string; percentage?: string }>; cities?: Array<{ id?: string; name?: string; percentage?: string }> } };
...
type FacebookInsights = {
  ...
  audience?: { netFollowers?: string; women?: string; men?: string; ageGroups?: FacebookMetric[]; topAgeGroup?: string; topCities?: string[]; locations?: { countries?: InsightLocation[]; cities?: InsightLocation[] } };
...
type MetricIcon = 'followers' | 'views' | 'interactions' | 'city' | 'growth' | 'age' | 'gender';
function metricIcon(icon: MetricIcon) { return { followers: String.fromCodePoint(0x1F465), views: String.fromCodePoint(0x25C9), interactions: String.fromCodePoint(0x2661), city: String.fromCodePoint(0x25C7), growth: String.fromCodePoint(0x2197), age: String.fromCodePoint(0x1F464), gender: String.fromCodePoint(0x25CB) }[icon]; }
...
export function ProfileSummaryCard({
  ...
  return <section className={'relative isolate overflow-hidden rounded-2xl border border-[#e8e2f1] bg-[linear-gradient(135deg,#fff_0%,#fdfbff_55%,#f7f2ff_100%)] shadow-[0_12px_34px_rgba(60,42,90,0.04)] ' + (portraitPanel ? '' : 'px-6 py-7 sm:px-8 sm:py-9 lg:px-10 lg:py-10')}>
    ...
    <div className={portraitPanel ? 'grid lg:grid-cols-[190px_minmax(0,1fr)_minmax(0,1fr)]' : 'grid gap-8 lg:grid-cols-2 lg:gap-10'}>
      {portraitPanel && (editablePhoto && creatorId && onPhotoUpdated ? <ProfilePhotoEditor
        creatorId={creatorId}
        photoUrl={photoUrl}
        initials={initials}
        alt={displayName ? `Portrait of ${displayName}` : 'Profile photo'}
        className="min-h-[260px] overflow-hidden border-b border-[#e8e2f1] bg-[#f1eaff] text-3xl font-semibold text-[#6731dc] sm:min-h-[300px] lg:min-h-full lg:border-b-0 lg:border-r"
        fallbackClassName="grid h-full w-full place-items-center bg-[linear-gradient(145deg,#f6f0ff_0%,#e6d8ff_100%)]"
        onPhotoUpdated={onPhotoUpdated}
      /> : <div className="min-h-[260px] overflow-hidden border-b border-[#e8e2f1] bg-[#f1eaff] text-3xl font-semibold text-[#6731dc] sm:min-h-[300px] lg:min-h-full lg:border-b-0 lg:border-r">
        {photoUrl ? <img src={photoUrl} alt={displayName ? `Portrait of ${displayName}` : 'Profile photo'} className="h-full w-full object-cover object-center" /> : <div className="grid h-full w-full place-items-center bg-[linear-gradient(145deg,#f6f0ff_0%,#e6d8ff_100%)]">{initials}</div>}
      </div>)}
      <div className={portraitPanel ? 'col-span-2 grid gap-8 px-6 py-7 sm:px-8 sm:py-9 lg:grid-cols-2 lg:gap-10 lg:px-10 lg:py-10' : 'contents'}>
      <div className="min-w-0">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:gap-6">
          {!portraitPanel && <div className="grid h-28 w-28 shrink-0 place-items-center overflow-hidden rounded-full border-[3px] border-[#d9c6f7] bg-[#f4effb] text-2xl font-semibold text-[#6c43b7] shadow-[0_5px_15px_rgba(73,47,120,0.08)] sm:h-32 sm:w-32 sm:text-3xl">
          ...
          <div className="min-w-0 pt-0.5">
            ...
            {showOwnerActions && <Link href="/profile/basic-information" className="mt-4 inline-flex text-[0.78rem] font-medium text-[#5f28dc] transition hover:text-[#4520a8]">Edit Basic Information <span aria-hidden="true" className="ml-1">→</span></Link>}
          </div>
        </div>
      ...
      <div className="min-w-0 border-t border-[#e9e3ee] pt-7 lg:border-l lg:border-t-0 lg:pl-10 lg:pt-0">
        <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-[#6330dc]">Content &amp; Niche</p>{showOwnerActions && <EditLink href="/profile/identity">Edit Creator Identity</EditLink>}</div>
        {hasContent ? <>
...
function AudienceSummary({ account }: { account: SocialAccount }) {
  ...
  const topAge = instagramAudience?.topAgeRanges?.[0]?.trim() || facebookAudience?.topAgeGroup?.trim() || '';
  const topCity = instagramAudience?.topCities?.[0]?.trim() || facebookAudience?.topCities?.[0]?.trim() || '';
...
export function PrimaryPlatformCard({ account }: { account: SocialAccount }) {
  ...
  const simplifiedPlatform = accountPlatform === 'instagram' || accountPlatform === 'facebook';
  const metrics = simplifiedPlatform
    ? [
      { label: 'Followers', value: account.audienceCount?.trim() ? formatCount(account.audienceCount) : '—', icon: 'followers' as const },
      { label: 'Views', value: views?.trim() ? formatCount(views) : '—', icon: 'views' as const },
      { label: accountPlatform === 'facebook' ? 'Engagement' : 'Interactions', value: interactions?.trim() ? formatCount(interactions) : '—', icon: 'interactions' as const },
    ]
    : [
...
export default function ProfilePreviewPage() {
  ...
  const loadPreview = async () => {
    ...
    try {
      ...
      if (identityResult.error || contentResult.error) throw new Error('We could not load your profile preview.');
    ...
    <main className="min-h-screen bg-[#fbfaff] px-3 py-3 text-[#19171d] sm:px-5 sm:py-5">
      <div className="mx-auto max-w-[1180px] overflow-hidden rounded-xl border border-[#ebe7f0] bg-white shadow-[0_12px_34px_rgba(60,42,90,0.04)]">
        <div className="px-7 pt-5 sm:px-9">
          <nav className="flex items-center justify-end gap-4" aria-label="Profile preview navigation">
