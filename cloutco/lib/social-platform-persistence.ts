...
export async function loadSocialAccounts(creatorId: string): Promise<Account[]> {
  ...
  const [instagram, facebook, youtube, ages, locations, breakdowns] = await Promise.all([
  ...
  if (instagram.error || facebook.error || youtube.error || ages.error || locations.error || breakdowns.error) throw new Error('We could not load your saved analytics.');
