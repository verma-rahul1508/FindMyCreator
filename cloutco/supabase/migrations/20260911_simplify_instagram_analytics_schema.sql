create table public.creator_instagram_top_age_ranges (
  id uuid primary key default gen_random_uuid(),
  snapshot_id uuid not null references public.creator_instagram_analytics(snapshot_id) on delete cascade,
  age_range text not null,
  display_order integer not null,
  created_at timestamptz not null default now(),
  constraint creator_instagram_top_age_ranges_snapshot_age_range_key unique (snapshot_id, age_range),
  constraint creator_instagram_top_age_ranges_display_order_check check (display_order between 1 and 2),
  constraint creator_instagram_top_age_ranges_age_range_check check (nullif(btrim(age_range), '') is not null)
);

create index creator_instagram_top_age_ranges_snapshot_id_idx
  on public.creator_instagram_top_age_ranges(snapshot_id);

create table public.creator_instagram_top_locations (
  id uuid primary key default gen_random_uuid(),
  snapshot_id uuid not null references public.creator_instagram_analytics(snapshot_id) on delete cascade,
  location_name text not null,
  display_order integer not null,
  created_at timestamptz not null default now(),
  constraint creator_instagram_top_locations_snapshot_location_name_key unique (snapshot_id, location_name),
  constraint creator_instagram_top_locations_display_order_check check (display_order between 1 and 5),
  constraint creator_instagram_top_locations_location_name_check check (nullif(btrim(location_name), '') is not null)
);

create index creator_instagram_top_locations_snapshot_id_idx
  on public.creator_instagram_top_locations(snapshot_id);

alter table public.creator_instagram_top_age_ranges enable row level security;
alter table public.creator_instagram_top_locations enable row level security;

create policy "Creators can manage own Instagram top age ranges"
on public.creator_instagram_top_age_ranges
for all to authenticated
using (exists (
  select 1
  from public.creator_instagram_analytics analytics
  ...
  where analytics.snapshot_id = creator_instagram_top_age_ranges.snapshot_id
...
create policy "Creators can manage own Instagram top locations"
on public.creator_instagram_top_locations
for all to authenticated
using (exists (
  select 1
  from public.creator_instagram_analytics analytics
  ...
  join public.creator_social_platforms platform_profile on platform_profile.id = snapshot.social_platform_id
  join public.creators creator on creator.id = platform_profile.creator_id
  where analytics.snapshot_id = creator_instagram_top_locations.snapshot_id
    and snapshot.platform = 'instagram'
    and creator.auth_user_id = auth.uid()
))
with check (exists (
...
revoke all on public.creator_instagram_top_age_ranges from public, anon, authenticated;
revoke all on public.creator_instagram_top_locations from public, anon, authenticated;
grant select, insert, update, delete on public.creator_instagram_top_age_ranges to authenticated;
grant select, insert, update, delete on public.creator_instagram_top_locations to authenticated;
...
begin
  ...
  for v_account in select value from jsonb_array_elements(p_accounts) loop
    ...
    if v_platform = 'instagram' and (
      ...
      or jsonb_typeof(v_insights#>'{audience,topAgeRanges}') <> 'array'
      or jsonb_array_length(v_insights#>'{audience,topAgeRanges}') not between 1 and 2
      or jsonb_typeof(v_insights#>'{audience,topCities}') <> 'array'
      or jsonb_array_length(v_insights#>'{audience,topCities}') not between 1 and 5
      or exists (select 1 from jsonb_array_elements_text(v_insights#>'{audience,topAgeRanges}') value where nullif(btrim(value), '') is null)
      or exists (select 1 from jsonb_array_elements_text(v_insights#>'{audience,topCities}') value where nullif(btrim(value), '') is null)
      or (select count(distinct lower(btrim(value))) from jsonb_array_elements_text(v_insights#>'{audience,topAgeRanges}') value) <> jsonb_array_length(v_insights#>'{audience,topAgeRanges}')
      or (select count(distinct lower(btrim(value))) from jsonb_array_elements_text(v_insights#>'{audience,topCities}') value) <> jsonb_array_length(v_insights#>'{audience,topCities}')
    ) then
    ...
    if v_platform = 'instagram' then
      insert into public.creator_instagram_analytics (
        ...
        v_snapshot_id, (v_insights#>>'{overview,views}')::bigint, (v_insights#>>'{overview,netFollowers}')::bigint, (v_insights#>>'{overview,interactions}')::bigint, (v_insights#>>'{overview,viewersTotal}')::bigint, (v_insights#>>'{overview,profileVisits}')::bigint, (v_insights#>>'{audience,women}')::numeric, (v_insights#>>'{audience,men}')::numeric
